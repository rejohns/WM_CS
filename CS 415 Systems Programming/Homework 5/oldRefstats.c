...
// debugging functions
void printbuff(void) {
	printf("\e[7mPRINTING BUFFER\e[0m\n");
	printf("BUFF SIZE: %d\n", buff -> size);
	buff -> curr = buff -> head;
	while (buff -> curr != NULL) {
		printf("%s\n", buff -> curr -> ipAddr);
		buff -> curr = buff -> curr -> next;
	}
	printf("\e[7mDONE PRINTING BUFFER\e[0m\n\n");
}
void printcache (void) {
	printf("\e[7mPRINTING CACHE\e[0m\n");
	printf("CACHE SIZE %d\n", cache -> size);
	cache -> curr = cache -> head;
	while (cache -> curr != NULL) {
		printf("%s\n", cache -> curr -> ipAddr);
		cache -> curr = cache -> curr -> next;
	}
	printf("\e[7mDONE PRINTING CACHE\e[0m\n\n");
}