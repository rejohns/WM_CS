/************************************************************************
 ...
 allows receiving and sending of resignation messages and appropriate
 game over communication. everything else is the same
*************************************************************************/
...
int argc; char **argv;
{
  ...
  bzero((char *)&inmsg, sizeof(inmsg));
  getmsg(Ofd, &inmsg);
  if (inmsg.type != HANDLE) protocol_error(HANDLE);
