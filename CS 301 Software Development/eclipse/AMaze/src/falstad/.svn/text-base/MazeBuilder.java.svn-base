...
public abstract class MazeBuilder {
	...
	public MazeBuilder(boolean deterministic){
		...
		{
			System.out.println("Homework2: functionality to make maze generation deterministic not implemented yet! Stop!");
			System.exit(0) ;
	 ...
	 * @return
	 */
	public static int grade_partition(Vector<Seg> sl, Seg pe) {
		int x  = pe.x;
		int y  = pe.y;
		int dx = pe.dx;
		int dy = pe.dy;
 ...
 */
	 ...
	 * @param remotey
	 */
	public void setExitPosition(int remotex, int remotey) {
		int bit = 0;
		if (remotex == 0)
			bit = Cells.CW_LEFT;
	 ...
	 */
	public void setStartPositionToCellWithMaxDistance() {
		int x;
		int y;
		int d = 0;
		for (x = 0; x != width; x++)
	 ...
	 * @param sl
	 */
	public void setPartitionBitForCertainSegments(Vector<Seg> sl) {
	 ...
	 * @return
	 */
	public BSPNode genNodes(Vector<Seg> sl) {
	 ...
	 * @return
	 */
	public Seg findPartitionCandidate(Vector<Seg> sl) {
		Seg pe = null ;
...
//			}
	 ...
	 * @return
	 */
	public int countNonPartitions(Vector<Seg> sl) {
		int result = 0 ;
		for (int i = 0; i != sl.size(); i++)
