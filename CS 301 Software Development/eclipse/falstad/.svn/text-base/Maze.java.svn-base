...
public class Maze extends Panel implements Runnable {
	 ...
	 */
	protected void redrawTitle(Graphics gc) {
	 ...
	 * @param gc graphics handler to manipulate screen
	 */
	protected void redrawFinish(Graphics gc) {
		gc.setColor(Color.blue);
		gc.fillRect(0, 0, view_width, view_height);
		gc.setFont(largeBannerFont);
		FontMetrics fm = gc.getFontMetrics();
	 ...
	 * @param gc graphics handler to manipulate screen
	 */
	protected void redrawGenerating(Graphics gc) {
		gc.setColor(Color.yellow);
		gc.fillRect(0, 0, view_width, view_height);
		gc.setFont(largeBannerFont);
		FontMetrics fm = gc.getFontMetrics();
	...
	public void run() {
		...
		else if (botName == "Curious Gambler") {
			RobotDriver solver = new CuriousGambler();
			try {
				solver.setRobot(r);
			} catch (UnsuitableRobotException e) {
				// TODO Auto-generated catch block
