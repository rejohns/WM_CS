 ...
 *
 * Modified heavily by Robert Johns on 9/10/2013 for cs426 at W&M
 * ------------------------------------------------------------------------- 
*/
...
{
int MAXIMUM = 60;
for (MAXIMUM; MAXIMUM <= 100; MAXIMUM++){
int MINIMUM = 1;
int bestMin;
double best = 500000000;
for(MINIMUM; MINIMUM <= 40; MINIMUM++){
  ...
  double setcst = 1000 * (sum.setup/index);
  double holdcst = 25 * (sum.holding/index);
  double shortcst = 700 * (sum.shortage/index);
  double depcst = setcst + holdcst + shortcst;
  if (depcst < best){
  	best = depcst;
  	bestMin = MINIMUM;
  }
...
//  printf("***************************************\n");
  //printf("%d & %6.2f & %6.2f & %6.2f & %6.2f\\\\\\hline\n",MINIMUM,setcst,shortcst,holdcst,depcst);
  fclose(fp);
  }
  printf("(%d, %d), ",MAXIMUM,bestMin);
  }
  return (0);
}
