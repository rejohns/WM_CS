...
bool GroupOfNumbers::partial_sort_select(long k, long & kth_smallest) const {
	...
	GroupOfNumbers temp_group(*this);
	temp_group.quicksort(0, k - 1);
	for(long j = k; j < _total; j++) {
      
		if (temp_group._group[j] < temp_group._group[k-1]) {
			temp_group._group[k-1] = temp_group._group[j];
			for(long p = k-1; p >0; p--) {
				if(temp_group._group[p] < temp_group._group[p-1]) {
					swap(temp_group._group[p], temp_group._group[p-1]);
				}
				else {
					break;
				}
...
bool GroupOfNumbers::min_heap_select(long k, long & kth_smallest) const {
	...
	long currentSize = temp_group.total() - 1;
	//comments by grader: for (int i = _total / 2; i >= 0; i--) {
	for (int i = _total / 2; i > 0; i--) {
		temp_group.percolateDown2(i, currentSize);
	}
	//comments by grader:
	//replcase the for loop with the following:
	//  for (int i = 0; i < k-1; i++) {
	//        swap(temp_group._group[0],temp_group._group[--currentSize]);
	//  }
	for (int i = 0; i < k; i++) {
		temp_group.set_element(0, temp_group.select_element(currentSize--));
		temp_group.percolateDown2(0, currentSize);
	}
...
void GroupOfNumbers::insertion_sort(long left, long right){
	for (int i = left; i <= right; i++) {
		...
		int j;
		//comments by grader:
		//for (j = i; j > left && tmp < _group[j - 1]; j--)
...
long GroupOfNumbers::median_of_median_of_5(long *tempArr, long left, long right) const{
	...
	if(temp_size>=5){
		...
		//comments by grader: (-2)
		//Recursively call linear select to find the median of medians.
...
void GroupOfNumbers::linear_select(long *temp_group, long left, long right, long k) const {
  	if(left + CutOff <= right){
		 /*-------------comments by grader:
< In your code, meadian_of_median_of_5 only returns the value of pivot, not the position of pivot,
<   You cannot guarantee that after the partition, position i stores the pivot, 
<  neither can you guarantee that the value of position i partition the array into small parts and large parts.               
< -------------------------*/
		...
		for(;;){
			//commens by grader:
			//while(temp_group[++i] < pivot && i<right){}
			//while(pivot < temp_group[--j] && j>left){}
		...
		//comments by grader:(-1)
		//delete the swap, you never store the pivot to this position.
		swap(temp_group[i], temp_group[right-1]);	
		if(k <= i) {//comments by grader:(-1) if(k <= i+1) {
  			linear_select(temp_group, left, i-1, k);
		}
		else if(k > i) {
			//comments by grader:(-1) linear_select(temp_group, i, right, k);
...
void GroupOfNumbers::percolateDown2(long hole, long currentSize) const {
	...
	// 	for(; hole * 2+1 < currentSize; hole = child) {
 	//	child = hole * 2+1;
 	//	if (child != currentSize-1 && _group[child + 1] < _group[child]) {
