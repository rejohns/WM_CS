...
partial_sort_select
-----------------------------
line 185: temp_group.quicksort(0, k);
Should be: temp_group.quicksort(0, k-1);
Point: -1
...
line 187 to 201:
	for (i = k; i < _total; i++) {
		if (temp_group.select_element(i) < temp_group.select_element(k - 1)) {
			temp1 = temp_group.select_element(i);
			for (int j = 0; j < k + c; j++) { //Every time loop k or more than k times.
				if(temp1 < temp_group.select_element(j)) {
					temp2 = temp_group.select_element(j);
					temp_group.set_element(j, temp1);
					temp1 = temp2;
    ...
    for(long j = k; j < _total; j++)
    {
      if (temp_group._group[j] < temp_group._group[k-1])
      {
		...
		for(long p = k-1; p >0; p--)
		{
			...
			{
				break; // No need to check further. As first k items are sorted.
			}
		}
      }
    }
...
-----------------------
Memory exception:
Weiss 6.12 use from index 1 of the array. 
So, your temp_group._group should have _total+1 allocation and start from temp_group._group[1] to temp_group._group[_total].

line 214: for (int i = _total / 2; i >= 0; i--)
Shoud be: for (int i = _total / 2; i > 0; i--)

You can also use Weiss 7.10 for min_heap. Only change a[child]<a[chid+1] to a[child]>a[child+1] and tmp<a[child] to a[child]>tmp.
And deletion will be
for (int i = 0; i < k-1; i++) 
	{
		temp_group[0] = temp_group[--_total];
		percolate_down_min(0, _total); //percolate_down_min is the converted function of Weiss 7.10.
	} 
