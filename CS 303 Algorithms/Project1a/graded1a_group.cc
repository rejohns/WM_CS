...
//GroupOfNumbers::remove(long const&) (group.cc:67)

//At line 67 (remove method), if the number is not in group array, loop exit after i==_total.
//But _group[i] means _group[_total] is uninitialised. 
//It can be  for (i = 0; (_group[i] != number) && (i < _total); i++){}
//So, for i==_total loop exit before checking _group[i]!=number
